...
| **BL-15** | Phase 5 hardening (P4-4 accepted caveats, must not evaporate): (1) R2 presigned PUTs get a server-enforced body-size condition — declared `content_length` is advisory today, a client can PUT a larger object; (2) ✅ 2026-07-13 (STEP 3): public rate limiting is Postgres-backed (`rate_limit_event` sliding window, DB clock, opportunistic prune — `hero.api.ratelimit.allow`) — survives restarts and multi-worker deploys; retires the per-process `SlidingWindowLimiter` | days | Phase 5 — (1) still open: single-worker pilot is safe, bites when a hostile client PUTs oversized bodies |
...
| **BL-20** | ✅ 2026-07-13 (STEP 4): **H4 — Timestamp source consistency** — root cause was NOT a UTC/local mix: migrations 0004/0005/0006/0008 passed `server_default` as a plain string, and Postgres constant-folds a string-literal `DEFAULT 'now()'` to the DDL-parse-time timestamp, freezing `created_at` on `ticket_event`/`conversation_message`/`rate_limit_event`/`app_user`/`building` at migration-run time (also silently defeating the BL-15 rate-limit window). Fixed: models use `text("now()")`, migration `0009` resets the five defaults, invariant test pins every `created_at` default to the real `now()` function + greps new source for the string form. Rows written under the frozen default carry unrecoverable timestamps | hours | Phase 5 STEP 4 |
| **BL-21** | ✅ 2026-07-13 (STEP 4): **H5 — Tenant-facing error UX** — `web/src/errors.ts` (`tenantErrorCopy`): 4xx = "was NOT sent", safe to retry; 5xx = "may still have gone through — refresh before resending" (the rehearsal's 500-on-success class); network = not sent, retry. Wired into Intake, Status (form answer), Chat opener and the conversation composer. Contractor Outcome hardened the same way: 20s timeout (a wedged server can't hang at "Filing…" forever), 401 mid-submit keeps the form + says the outcome was NOT filed (no silent bounce-to-login losing BL-0 data), 5xx/no-response copy says to check the ticket list before refiling | days | Phase 5 STEP 4 |
| **BL-22** | ✅ 2026-07-13 (DEC-26): **Mid-chat photo attach** — `POST /public/status/{slug}/presign` (status-link holders; same image-only/size caps, keys minted under the ticket's building so the shared `public-intake/` validation holds) + `POST …/messages` accepts `photos`: media rows via existing pointers (INV-3) and one tenant `kind="photo"` transcript row appended BEFORE the text turn (visible in chat + operator ledger). A redirected message (DEC-24) keeps nothing. Photos do NOT feed a run already in flight — that is BL-23 | hours | Phase 5 (with DEC-26 retheme) |
| **BL-23** | **Mid-run evidence injection**: photos/messages arriving while a run is in flight should be able to feed DIAGNOSE as new evidence. Touches the single-resume-path rule (spec §4) — needs its own design pass (where the run re-enters, how VERIFY/SAFETY_GATE re-trigger, how the ledger records the injection) before any code. Until then mid-chat photos land in media + transcript only (BL-22); the operator sees them in the ledger | 1–2 w | Do NOT bolt onto the resume path ad hoc |
...
| DEC-26 | 2026-07-13 | **Tenant chat surfaces adopt the "Hero Diagnosis" visual language** (founder-supplied Figma Make export, `design/nova-reference/`): yellow `#F5C518` mobile frame, app-bar header (history-back, hidden without history; kebab omitted), avatar + dark `#2C2C2C` Nova bubbles with flat top-left corner, right-aligned white tenant bubbles (undefined in the mock — our call), per-message timestamps, pill composer with circular camera/send buttons. **Interpretation record: the mock defined exactly ONE static screen** — every state it left undefined (escalation/redirect/capped banners, working copy + polling, H5 error copy, contact capture, photo chips, form-ticket status view) keeps current behavior in the new skin. Deviations from the mock: greeting copy replaced (the mock's "home services booking assistant … HVAC, plumbing, cleaning, or electrical" violates DEC-24 verbatim); mic + "tap to hold to speak" + speaker/TTS icons omitted (DEC-25 — no dead controls on a tenant surface); WCAG AA contrast fixes — timestamps `#B8860B`→`#6B4E00` (2.0:1→4.7:1 on yellow), placeholder gray-400→`#6B7280` (2.5:1→4.8:1 on white). Design bundles are visual reference only: zero dependencies imported, implemented in the existing `web/` stack, never wired into any build. | The founder's design direction owns look-and-feel; PRD invariants own behavior. Recording the one-screen provenance + each deviation keeps "the design said so" from ever overriding DEC-24/DEC-25/INV-1 silently. |
