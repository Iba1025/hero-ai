...
- **Observability:** self-hosted Langfuse (INV-2), every run traced. Use Langfuse's LLM-as-judge
  evaluators (Ragas-backed) as part of the eval harness (BL-3) — validate judge–human agreement
...
| **BL-4** | ✅ 2026-07: Complexity routing in TRIAGE — VLM triage (primary tier, DEC-18) with deterministic INV-1 fail-safes (DEC-21); `complexity=="simple"` routes to `retrieve_fast` graph node (BM25-only top 5, no rerank); CLARIFY loop always re-enters full path; eval reports fast/full path split (latency + cost) | ~1 wk | 3–10× token, 2–5× latency cost of full path; unit economics |
