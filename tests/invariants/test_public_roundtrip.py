...
async def test_public_intake_clarify_answer_roundtrip(
    ...
    # 1. Tenant files the problem → run interrupts at CLARIFY.
    ...
    # 3. Tenant answers → resumed through the single resume path → completes.
    resp = await client.post(f"/public/status/{status_slug}/answer", json={"answer": ANSWER})
    assert resp.status_code == 200
    assert resp.json()["state"] == "being handled"
